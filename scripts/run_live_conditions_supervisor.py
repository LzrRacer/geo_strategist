"""Durable supervisor for rerunning the C0-C14 live condition track.
...
from geo_strategist.experiments.condition_registry import CONDITION_ORDER  # noqa: E402

# Derived from the registry (the executable source of truth) rather than
# hardcoded, so this list cannot drift out of sync with a future renumbering.
...
def supervisor_env() -> dict[str, str]:
    ...
    env.setdefault("C13_NUM_DRAFTS", "3")
    env.setdefault("C13_AGENT_STEPS", "8")
    env.setdefault("C13_VARIANTS_PER_BRANCH", "1")
    env.setdefault("C13_MAX_DEBUG_DEPTH", "0")
    env.setdefault("C13_CONCURRENCY", "1")
