...
# ---------------------------------------------------------------------------
# C11-C13 candidate-level qualitative deliberation layer
#
# Adds an explicit "agent writes a qualitative assessment" step ahead of
# reviewer critique. ``CandidateQualitativeAssessment`` and
# ``CandidateAssessmentReview`` are stored verbatim as
# ``candidate_qualitative_assessments.jsonl`` / ``candidate_assessment_reviews.jsonl``;
# author responses and provenance filtering continue to reuse
# ``AuthorResponse`` / ``CandidateReviewPacket`` / ``judge_candidate_review_packet``.
# ---------------------------------------------------------------------------

class AssessmentContent(BaseModel):
    why_selected: str = ""
    demand_supply_case: str = ""
    financial_case: str = ""
    access_case: str = ""
    site_feasibility_case: str = ""
    main_uncertainties: list[str] = Field(default_factory=list)
    what_would_falsify: list[str] = Field(default_factory=list)


class CandidateQualitativeAssessment(BaseModel):
    condition_id: str
    candidate_id: str
    rank: int | None = None
    assessment: AssessmentContent
    evidence_refs: list[str] = Field(default_factory=list)
    model_role: str = "author"
    model_name: str = ""
    created_at: str = ""


class ReviewScope(BaseModel):
    saw_candidate_dossier: bool = True
    saw_agent_assessment: bool = True
    saw_full_candidate_universe: bool = False
    saw_model_identity: bool = False
    saw_condition_identity: bool = False


class CandidateAssessmentReview(BaseModel):
    condition_id: str
    candidate_id: str
    reviewer: str
    findings: list[ReviewerFinding] = Field(default_factory=list)
    review_scope: ReviewScope = Field(default_factory=ReviewScope)
    model_role: str = "reviewer"
    model_name: str = ""
    created_at: str = ""
