...
def run_condition_proposals_command(
    conditions: str = typer.Option(
        "C0,C1,C2,C3,C4,C5,C6,C7,C8,C9,C10,C11,C12,C13", "--conditions",
        help="Comma-separated condition groups to run (canonical C0-C13).",
    ),
    ...
    candidate_deliberation: bool = typer.Option(
        ...
        help="Run the candidate-level critical-reviewer deliberation pipeline "
             "after each non-deterministic condition's base slate.",
    ),
    ...
    auto_agentic_harness: bool = typer.Option(
        ...
        help="Opt in to non-interactive CLI execution for C2/C3/C5-C8 adapters that support it.",
    ),
) -> None:
    """Generate a proposal report per C0-C13 condition plus the E13 comparison."""
...
def write_manual_harness_prompts_command() -> None:
    """Write C2/C3 handoff prompts and C5-C8 Skills launcher files."""

    from geo_strategist.harnesses.prompts import write_manual_harness_prompts

    written = write_manual_harness_prompts(Path("."))
...
def run_condition_comparison_judge_command(
    ...
    allow_live_judge: bool = typer.Option(
        ...
        help="Use the live LLM judge in addition to the structural judge. "
             "Makes live API calls by default; pass --structural-judge-only "
             "(or set E13_DISABLE_LIVE_JUDGE=1) for a fully offline run.",
    ),
) -> None:
    """Run the E13 condition-comparison judge across available C0-C13 records."""
