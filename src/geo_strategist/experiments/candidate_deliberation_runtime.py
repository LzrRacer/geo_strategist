...
shared, external post-review augmentation applied on top of a condition's
own algorithm. It is skipped by ``condition_supports_candidate_deliberation``
for algorithms that must remain a true single-pass/deterministic baseline:
the deterministic Python baseline (C0) and every Vanilla LLM condition
(C1-C4, ``algorithm == "vanilla_llm"``) — identified by ``spec.algorithm``,
not by condition group or runner, since C2 and C3 dispatch through a
manual-harness runner despite being Vanilla conditions. An advanced condition
that implements its own internal candidate-review step is unaffected: this
module only gates the external augmentation, never an algorithm's own
...
# Algorithms that must remain true single-pass/deterministic baselines and
# never receive the shared post-hoc candidate-review augmentation. Checked
# against spec.algorithm (not spec.runner or spec.group): C2 and C3 are
# Vanilla LLM conditions (algorithm == "vanilla_llm") dispatched through the
# "manual_harness" runner, so a runner- or group-based check would miss them.
_NO_CANDIDATE_DELIBERATION_ALGORITHMS: frozenset[str] = frozenset({
...
def condition_supports_candidate_deliberation(spec: ConditionSpec) -> bool:
    if spec.runner in _DETERMINISTIC_RUNNERS:
        return False
    if spec.algorithm in _NO_CANDIDATE_DELIBERATION_ALGORITHMS:
        return False
    return True
