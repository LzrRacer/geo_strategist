...
One shared builder serves every condition (C0-C13 and manual-harness
...
def _review_paragraph(proposal: dict[str, Any],
                      ...
                      *,
                      ...
                      review_threads: list[dict[str, Any]] | None = None,
                      candidate_deliberation_state: str = "not_run") -> str:
    parts: list[str] = []
...
def render_candidate_deliberation_section(
    proposals: list[dict[str, Any]],
    assessment_by_candidate_id: dict[str, dict[str, Any]],
    packet_by_candidate_id: dict[str, dict[str, Any]],
) -> str:
    """The C11-C13 candidate-level qualitative deliberation section.

    One block per candidate that has an agent assessment: the assessment
    itself, the (provenance-filtered) reviewer findings that survived the
    consistency judge, and the author's response to them. Only ``retain`` /
    ``retain_with_major_due_diligence`` outcomes are ever reflected here --
    candidate replacement, when enabled, is reported separately in the
    packet's ``replacement_decision``.
    """

    from geo_strategist.reporting import markdown_table

    if not assessment_by_candidate_id:
        return ""
    lines = ["## Candidate-level qualitative deliberation", ""]
    for proposal in proposals:
        candidate_id = str(proposal.get("candidate_id"))
        assessment_row = assessment_by_candidate_id.get(candidate_id)
        if not assessment_row:
            continue
        content = assessment_row.get("assessment") or {}
        packet = packet_by_candidate_id.get(candidate_id) or {}
        findings = packet.get("reviewer_findings") or []
        responses_by_id = {
            str(r.get("finding_id")): r for r in packet.get("author_responses") or []
        }
        lines.extend([f"### Candidate: {candidate_id}", "", "#### Agent assessment", ""])
        lines.extend([
            f"- Why selected: {content.get('why_selected') or 'not_available'}",
            f"- Demand/supply case: {content.get('demand_supply_case') or 'not_available'}",
            f"- Financial case: {content.get('financial_case') or 'not_available'}",
            f"- Access case: {content.get('access_case') or 'not_available'}",
            f"- Site feasibility case: {content.get('site_feasibility_case') or 'not_available'}",
            "- Main uncertainties: "
            + ("; ".join(content.get("main_uncertainties") or []) or "none recorded"),
            "- What would falsify this recommendation: "
            + ("; ".join(content.get("what_would_falsify") or []) or "none recorded"),
            "",
        ])
        lines.extend(["#### Reviewer findings", ""])
        if findings:
            lines.append(markdown_table(
                ["reviewer", "severity", "issue", "recommendation", "status"],
                [[f.get("reviewer_id"), f.get("severity"), str(f.get("issue") or "")[:160],
                  f.get("recommendation"),
                  (responses_by_id.get(str(f.get("finding_id"))) or {}).get(
                      "response_status", "unresolved")]
                 for f in findings],
            ))
        else:
            lines.append(
                "No reviewer findings survived provenance/consistency checks for this candidate.")
        lines.extend(["", "#### Author response", ""])
        if findings:
            accepted = [
                f for f in findings
                if (responses_by_id.get(str(f.get("finding_id"))) or {}).get("response_status")
                == "accepted"
            ]
            unresolved = [
                f for f in findings
                if f.get("severity") in ("blocking", "major")
                and (responses_by_id.get(str(f.get("finding_id"))) or {}).get("response_status")
                != "accepted"
            ]
            due_diligence_items = sorted({
                item for response in packet.get("author_responses") or []
                for item in response.get("added_due_diligence") or []
            })
            lines.extend([
                f"- Accepted findings: {len(accepted)} of {len(findings)}",
                "- Revisions made: " + ("; ".join(due_diligence_items[:5]) or "none recorded"),
                "- Unresolved risks: "
                + ("; ".join(str(f.get("issue") or "")[:120] for f in unresolved[:3]) or "none"),
                "- Due diligence items: " + ("; ".join(due_diligence_items) or "none recorded"),
            ])
        else:
            lines.append("No author responses recorded.")
        lines.append("")
...
def render_candidate_deliberation_summary(summary: dict[str, Any]) -> str:
    """Aggregate counts for the C11-C13 candidate deliberation layer.

    Kept as its own small table (not folded into the per-candidate section
    above) so a reader -- or E13 -- can see coverage/quality at a glance
    without re-deriving it from every candidate block.
    """
    ...
    if not summary:
        return ""
    lines = [
        "## Candidate deliberation summary", "",
        markdown_table(
            ["metric", "value"],
            [[key.replace("_", " "), value] for key, value in summary.items()],
        ),
        "",
    ]
    return "\n".join(lines) + "\n"
