...
def check_antigravity() -> HarnessStatus:
    """Lightweight availability check only — `agy --version` succeeding is
    NOT evidence that a full agentic run will complete (headless mode's
    inability to service approval prompts, and the default 5-minute
    --print-timeout, were both found to break real runs independent of the
    binary being present and working). Use
    `geo_strategist.harnesses.antigravity_support.run_antigravity_preflight`
    (wired to `agy-strategist antigravity-preflight`) before a real run."""
    ...
    return HarnessStatus(
        ...
        logged_in=None,
        model=os.environ.get("C9_MODEL", os.environ.get("C5_MODEL", gemini_model)),
        ...
        notes=[
            ...
            "outputs/condition_proposals/live/manual_harness/ for C9.",
            "This check only confirms the binary responds to --version; run "
            "`.venv/bin/python -m geo_strategist.cli antigravity-preflight` "
            "for a real bounded smoke test before a long automated run.",
        ],
    )
...
def check_opencode() -> HarnessStatus:
    ...
    if not (has_opencode_credential or has_opencode_env):
        status.notes.append(
            ...
            "OPENCODE_API_KEY is not present; C12 automation may fail until "
